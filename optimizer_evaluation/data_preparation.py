...
X_boston = pd.DataFrame(data=data_boston.data,
...
X_breast_cancer = pd.DataFrame(
    data=data_breast_cancer.data, columns=data_breast_cancer.feature_names)
y_breast_cancer = pd.DataFrame(
    data=data_breast_cancer.target, columns=['class'])
...
X_train_boston, X_test_boston, y_train_boston, y_test_boston = train_test_split(
    X_boston, y_boston, test_size=0.2)
